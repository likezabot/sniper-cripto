...
export const calculateMACD = (prices: number[]): { macd: number; signal: number; histogram: number } => {
  ...
  const signal = calculateEMA([...Array(8).fill(macd), macd], 9); // Simulação simplificada
...
// Função para calcular ADX (Simplified)
export const calculateADX = (highs: number[], lows: number[], closes: number[], period: number = 14): number => {
  ...
  let adxValue = 0;
  const trend = closes[closes.length - 1] > closes[closes.length - period - 1] ? 1 : -1;
  const volatility = Math.max(...highs.slice(-period)) - Math.min(...lows.slice(-period));
  const avgVolatility = volatility / period;
  
  // Simular um valor de ADX baseado na tendência e volatilidade
  adxValue = 20 + (trend * avgVolatility * 10);
...
export const generateSignals = (
  ...
  const macdScore = histogram > 0 ? 70 + (histogram * 30) : 30 + (histogram * 30);
  ...
  const momentumScore = momentum > 0 ? 50 + (momentum * 2) : 50 + (momentum * 2);
  ...
  const indicators: Indicator[] = [
    { name: 'RSI', value: Math.abs(rsiScore), isPositive: rsiScore > 50 },
    { name: 'MACD', value: Math.abs(macdScore), isPositive: macdScore > 50 },
    { name: 'EMA', value: Math.abs(emaScore), isPositive: emaScore > 50 },
    { name: 'VWAP', value: Math.abs(vwapScore), isPositive: vwapScore > 50 },
    { name: 'S/R', value: Math.abs(supportResistanceScore), isPositive: supportResistanceScore > 50 },
    { name: 'Engolfo', value: Math.abs(engulfingScore), isPositive: engulfingScore > 50 },
    { name: 'Volume', value: Math.abs(volumeScore), isPositive: volumeScore > 50 },
    { name: 'Bollinger', value: Math.abs(bollingerScore), isPositive: bollingerScore > 50 },
    { name: 'ADX', value: Math.abs(adxScore), isPositive: adxScore > 50 },
    { name: 'Momentum', value: Math.abs(momentumScore), isPositive: momentumScore > 50 }
  ];
